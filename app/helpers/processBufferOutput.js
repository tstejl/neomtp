...
export const processMtpBuffer = async ({ error, stderr }) => {
  const result = await _processKalamMtpBuffer({ stderr });
  const noMtpError = isNoMtpError({ stderr });

  if ((stderr || error) && !noMtpError) {
    log.doLog(
      `MTP buffer o/p logging;${EOL}Raw error: ${(
        error ?? ''
      ).toString()}${EOL}Processed error: ${result.error}${EOL}Error type: ${
        stderr ?? ''
      }`,
      'processMtpBuffer',
      null,
